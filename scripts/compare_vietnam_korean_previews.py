...
SUSPICIOUS = [
    '전생','학금','장학비','도저','도천','토주','후천 시립','지아토ング','비엔나티움',
...
for rel in matched:
    ...
    page_reports[rel]={
        'source_metrics': metrics(s),
        'korean_metrics': metrics(k),
        'missing_internal_hrefs_in_korean': sorted(set(sh)-set(kh)),
        'extra_internal_hrefs_in_korean': sorted(set(kh)-set(sh)),
        'source_youtube_ids': youtube_ids(s),
